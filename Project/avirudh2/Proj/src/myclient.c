...
int   maxfd;
	...
	printf("\n My Node Details: %s:%d", selfnode.node_name, selfnode.node_port);
            ...
            while( token != NULL ) 
             ...
             if(strstr(selfnode.node_name,node_nbr[count]) != NULL){
               ...
               temp_count++;
                ...
                strcpy(Neighbors[temp_count],node_nbr[count]);
                for(i=0;i<node_cnt;i++){
                  if(strstr(names[i],Neighbors[temp_count])!=NULL){
                    metric[i] = distance[count];
                  }
...
// ***************  TILL HERE *********************************************                 
         ...
         if(FD_ISSET(sockfd,&readfd)){
             n = recvfrom(sockfd, read_buffer, MAXBUFSIZE, 0, (struct sockaddr *) &recv_addr, &addrlen);
          }
